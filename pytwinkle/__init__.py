...
class Twinkle():
    def __init__(self, callback):
        ...
        self.keep_session_process = Process(target=self.keep_session, args=[
                                            self.session_expires])  # m.group(2)
        ...
        self.states = ['new_msg', 'incoming_call', 'answered_call']
    ...
    def check_registration_succeeded(self, line):
        is_reg = False
        m = re.search(
            r'(.*): registration succeeded \(expires \= (.*) seconds\)', line)
        if m:
            is_reg = True
            if self.callback:
                self.callback("registration_succeeded", m.group(1), m.group(2))
    ...
    def check_msg(self, line):
        ...
        if re.search('Received message', line):
            ...
            if m:
                ...
                # print(self.msg)
                if self.callback:
                    self.callback("new_msg", self.msg)
                self.msg = {'from': None, 'to': None, 'msg': None}
    ...
    def check_incoming_call(self, line):
        ...
        if re.search(r'Line [0-9]: incoming call', line):
            ...
            if m:
                ...
                if self.callback:
                    self.callback("incoming_call", self.incoming_call)
                self.incoming_call = {'from': None, 'to': None}
        return is_call
    ...
    def check_cancelled_call(self, line):
        ...
        if m:
            is_can = True
            if self.callback:
                self.callback("cancelled_call", m.group(1))
        return is_can
    ...
    def check_answered_call(self, line):
        ...
        if re.search(r'Line [0-9]: far end answered call.', line):
            ...
            if m:
                ...
                self.state['line'] = 0
                if self.callback:
                    self.callback("answered_call", self.answered_call)
    ...
    def check_ended_call(self, line):
        ...
        if m:
            is_reg = True
            if self.callback:
                self.callback("ended_call", m.group(1))
        return is_reg
    ...
    def check_failed_call(self, line):
        ...
        if m:
            is_reg = True
            if self.callback:
                self.callback("failed_call", m.group(1))
        return is_reg
    ...
    def check_dtmf_received(self, line):
        ...
        if m:
            is_reg = True
            if self.callback:
                self.callback("dtmf_received", m.group(1), m.group(2))
        return is_reg
    ...
    def message(self, uri, msg):
        self.twinkle_process.stdin.write(
            str.encode("message %s %s\n" % (uri, msg)))
        self.twinkle_process.stdin.flush()
